...
class AutoSchedulerConfig:
    ...
    def __post_init__(self):
        ...
        if isinstance(self.env_vars, dict):
            self.env_vars = EnvVar(**self.env_vars)
...
def dict_to_config(parsed_yaml: Dict[Any, Any]) -> AutoSchedulerConfig:
    ...
    env_vars = EnvVar(**parsed_yaml["env_vars"])
    return AutoSchedulerConfig(tiramisu, dataset, ray, experiment, lstm_policy, env_vars)
...
class Config(object):
    ...
    @classmethod
    def init(self, config_yaml="./conf/config.yaml"):
