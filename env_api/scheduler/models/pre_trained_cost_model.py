...
class Pre_trained_model(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.device = device
        self.encoder = pre_train_encoder.to(self.device).eval()
        embedding_size = comp_embed_layer_sizes[-1]
        self.is_fine_tune = False
    ...
    def forward(self, tree_tensors):
        ...
        vectors = comps_tensor_vectors.to(self.device).view(batch_size * num_comps, -1) # No need to reshape this tensor since we transformed it when loading the data
        ...
        expr_tree = functions_comps_expr_tree.view((batch_size* num_comps, len_sequence*len_vector))
        x = torch.cat((first_part, vectors, third_part, expr_tree), dim=1)#.view(batch_size * num_comps, -1)

        # if is after the pre-defined epoch, allow the weights in encoder to be updated
        if self.training and self.is_fine_tune:
            x = self.encoder(x).view(batch_size, num_comps, -1)
        else: # otherwise set it to eval
            x = self.encoder.eval()(x).view(batch_size, num_comps, -1)
        ...
        return self.LeakyReLU(out[:, 0, 0]), x.detach().view(-1)
